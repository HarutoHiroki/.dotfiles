...
# Set initial wallpaper (running in background to ignore errors and prevent hanging)
echo -e "${STY_CYAN}Setting initial wallpaper...${STY_RST}"
run_cmd "${XDG_CONFIG_HOME}/quickshell/ii/scripts/colors/switchwall.sh --image </dev/null >/dev/null 2>&1 &"
...
_CURRENT_HOSTNAME="$(hostname -s)"
_CURRENT_USER="$(id -un)"
if [[ ( "$_CURRENT_HOSTNAME" == "svartalfheim" || "$_CURRENT_HOSTNAME" == "vanaheim" ) && \
      ( "$_CURRENT_USER" == "harutohiroki" || "$_CURRENT_USER" == "njord" ) ]]; then
  ...
  echo -e "${STY_YELLOW}Skipping personal GPG/Git config (hostname: $_CURRENT_HOSTNAME, user: $_CURRENT_USER)${STY_RST}"
fi
